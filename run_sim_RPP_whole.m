...
for i = 1:num_vars
    ylower(i) = 0.9*min(min(X_m(i,:,exact_per,exact_scen1)), min(X_f(i,:,exact_per,exact_scen1)));
    yupper(i) = 1.1*max(max(X_m(i,:,exact_per,exact_scen1)), max(X_f(i,:,exact_per,exact_scen1)));
...
for i = 1:7
    ...
    for j = 1:last_plot
        ...
        plot(s1(i,j), t,X_m((i-1)*15+j,:,exact_per,exact_scen1),'b' , ...
                      t,X_f((i-1)*15+j,:,exact_per,exact_scen1),'r');
...
RPPplot(1        :tchange) = RPP(1,exact_scen1);
RPPplot(tchange+1:tend+1 ) = RPP(1,exact_scen1) + RPP_per(exact_per);
...
% RPP
RPP_m = RPP(1,exact_scen1) + RPP_per; RPP_f = RPP(2,exact_scen1) + RPP_per;
