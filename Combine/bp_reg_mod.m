...
function f = bp_reg_mod(t,x,x_p,pars,...
                        tchange,varargin)
...
else % rat % will be rewritten to use varargin
...
if spe == 1 %human
    ...
    eta_ptwreab_eq   = pars(43);
    eta_dtwreab_eq   = pars(44);
    eta_cdwreab_eq   = pars(45);
end
...
if spe == 1 %human
    ...
    sodin_A = fixed_vars_par(1);
end
f(2 ) = alpha_map - ( 0.5 + 1 / (1 + exp((P_ma - sodin_A) / 15)) );
% alpha_rap
...
if spe == 1 %human
    ...
    elseif NSAID > 1
        ...
        f(10) = Sigma_tgf - ( 0.3408 + 3.449 / (3.88 + exp(((1-kappa_f_md)*Phi_mdsod  - 3.859*SF_S)/(-0.9617*SF_S))));
    end
else % rat
    if     t < tchange
...
if spe == 1 %human
    ...
    gamma_filsod_a = 0.8;
    gamma_filsod_b = fixed_var_pars(3);
end
f(14) = gamma_filsod - ( gamma_filsod_a + 0.3 / (1 + exp((Phi_filsod - gamma_filsod_b)/(138 * SF_S) )) );
% gamma_at
if spe == 1 %human
    gammaat_c = 1.1103;
    gammaat_d = 0.95;
...
% eta_cdsodreab
f(23) = eta_cdsodreab - ( eta_cdsodreab_eq * lambda_dt * lambda_anp * lambda_al); %lambda_al was not here in rat equations
% lambda_dt
if spe == 1 %human
    lambdadt_a = 0.82;
    lambdadt_b = 0.39;
...
if spe == 1 %human
    ...
    lambda_al_a = (387^0.06);
end
f(26) = lambda_al - ( 1/lambda_al_a * C_al^0.06 );
% Phi_usod
if spe == 1 % human
...
if spe == 1
    ...
    vas_f_a = -1/SSdata_input(33) * log(1/11.312);
    vas_f_b = 100;
end
f(36) = vas_f - ( (11.312 * exp(-Phi_co * vas_f_a)) / vas_f_b );
% vas_d
f(37) = vas_d - ( vas * K_vd );
% R_a
...
if spe == 1 % human
    ...
    a_auto_a = fixed_var_pars(5);
end
f(44) = a_auto - ( 3.0042 * exp(-a_auto_a * P_ma) );
% a_chemo
...
if spe == 1 %human
    ...
    N_adhs_a = fixed_var_pars(6);
end
f(49) = N_adhs - ( N_adhs_eq * (max( 0, C_sod - N_adhs_a) + max( 0, epsilon_aum - 1 ) - delta_ra) / 3 );
% delta_ra
...
if spe == 1 %human
    if     strcmp(gender,'male')
        if  NSAID > 0
            f(53) = nu_mdsod - ( 0.2262 + 83.4095/(77.6196+exp(((1-kappa_f_md)*Phi_mdsod - 1.731*SF_S)/0.6056*SF_S)));
        else 
            f(53) = nu_mdsod - ( 0.2262 + 28.04 / (11.56 + exp(((1-kappa_f_md)*Phi_mdsod - 1.731 * SF_S) / (0.6056 * SF_S) )) );
        end
    elseif strcmp(gender,'female')
        if NSAID > 0
            f(53) = nu_mdsod - ( 0.2262 + 83.4095/(77.6196+exp(((1-kappa_f_md)*Phi_mdsod - 1.637*SF_S)/0.6056*SF_S)));
        else
           f(53) = nu_mdsod - ( 0.2262 + 28.04 / (11.56 + exp(((1-kappa_f_md)*Phi_mdsod - 1.637 * SF_S) / (0.6056 * SF_S) )) );
        end
    end
else % rat
...
% AngII
f(68) = AngII_p - ( k_AngII + ((1-kappa_ACEI)*c_ACE + c_Chym) * AngI - (c_ACE2 + c_IIIV + c_AT1R + c_AT2R) * AngII - log(2)/h_AngII * AngII );
% AT1R
...
if spe == 1 % human
    ...
    sigmamyo_c = 9;
    sigmamyo_e = 62;
    if myo_ind > 0 % impaired myogenic
        sigmamyo_a = 1.2;
        sigmamyo_b = 0.3;
    end 
