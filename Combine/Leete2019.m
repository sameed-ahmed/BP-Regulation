    ...
    for AA = 1:2
        for c =  0:1 % loop through healthy and impaired myogenic response
            ...
            for i = [1,4,11] %loop through models to simulate
                for j=1%:length(models) % loop through data for initial guesses for solver
                    IG = sprintf('IGData/human_%s_ss_%s_%s_%s_rsna%s.mat',gender{gg},num2str(models(j,1)),num2str(models(j,2)),num2str(models(j,3)),num2str(rsna(AA)));
                    [exitflag, imag] = solve_ss_numerical(human,gg,IG,'ACEI',models(i,1),'furosemide',models(i,2),'NSAID',models(i,3),'RSNA',rsna(AA),'Myogenic Response',c);
    ...
    for AA = 1:2 %loop through normo/hypertension
        for c = 0:1 % loop through healthy and impaired myogenic response
            for w = 0:1 % loop through normal and low water intake
                ...
                for i = 1:15%1:length(models) %loop through drug treatments to simulate
                    disp({char(9),'ACEI',num2str(models(i,1)),'furosemide',num2str(models(i,2)),'NSAID',num2str(models(i,3))})
                    IG = sprintf('Data_ALD/human_%s_ss_%s_%s_%s_rsna%s%s.mat',gender{gg},num2str(0),num2str(0),num2str(models(i,3)),...
                                 num2str(rsna(AA)),myo_ending{c+1});
                    run_sim_treatments(human,gg,IG,'ACEI',models(i,1),'furosemide',models(i,2),'NSAID',models(i,3),...
                                                          'RSNA',rsna(AA),'Myogenic Response',c,'Water Intake', w);
                end
            end
        end
    end
