...
for gg = sex_to_run %sex
    for mm= treatments_to_run 
        for name_end = 1:length(endings)
            ...
            filename2 = sprintf( filename,'Data/',species{human+1},gender{gg},num2str(models(mm,1)),num2str(models(mm,2)),num2str(models(mm,3)),num2str(hyp(AA)),endings{name_end});
            %disp({mm,filename2})
            load(filename2, 't','x');
    ...
    for i=1%:length(plot_these)
        ...
        savefig(f, sprintf('Figures/%s_%s_riskfactors.fig',gender{gg},savenames{i}))
        saveas(f,sprintf('Figures/%s_%s_riskfactors.png',gender{gg},savenames{i}))
    end
end
