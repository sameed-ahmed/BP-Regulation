...
f(14) = gamma_filsod - ( 0.85 + 0.3 / (1 + exp((Phi_filsod - fixed_var_pars(3))/(138 * SF_S) )) );
% gamma_at
...
% -----------------------------------------------------------------------
if     strcmp(gender,  'male')
lambdadt_b = 0.2555; % new
% lambdadt_b = 0.352; % old
...
% ---
% lambdadt_c = 2; % <= 90%
lambdadt_c = -24 / SF_S / (log(lambdadt_b / (1 / eta_cdsodreab_eq - 0.82) - 1) - log(lambdadt_b / (1 - 0.82) - 1));
end
...
% -------------------------------------------------------------------------
% phiwin_a = 0.8;
% phiwin_b = SSdata(47) + 1/phiwin_a * log(0.002313 / (0.001) - 1);
% f(28) = Phi_win - ( 0.002313 / (1 + exp(-phiwin_a * (C_adh - phiwin_b))) + (SSdata(28) - 0.001) );
% -------------------------------------------------------------------------
% f(28) = Phi_win - ( SSdata(28) );
% -------------------------------------------------------------------------
phiwin_a = 0.8;
phiwin_b = SSdata(47) + 1 / phiwin_a * log(0.002313*15 / SSdata(28) - 1);
...
f(49) = N_adhs - ( (max( 0, C_sod - fixed_var_pars(6)) + max( 0, epsilon_aum - 1 ) - delta_ra) / 3 );
% delta_ra
...
% N_als
f(70) = N_als - ( xi_ksod * xi_map * xi_at );
% xi_ksod
...
% R_sec
f(77) = R_sec - ( N_rs * nu_mdsod * nu_rsna * nu_AT1 );
% PRC
...
% Sigma_myo
% f(88) = Sigma_myo - ( 0.6 + 1.4 / ( 1 + (5/2) * exp(-0.5 * (P_gh - fixed_var_pars(10))) ) );
% f(88) = Sigma_myo - ( 0.8 + 1.2 / ( 1 + (5/1) * exp(-0.4 * (P_gh - fixed_var_pars(9))) ) );
f(88) = Sigma_myo - ( 0.8 + 1.5 / ( 1 + (13/2) * exp(-0.6 * (P_gh - fixed_var_pars(9))) ) );
% f(88) = Sigma_myo - ( 1 );
