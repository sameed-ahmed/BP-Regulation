...
f(49-1) = N_adhs - ( N_adhs_eq * (max( 0, C_sod - fixed_var_pars(6)) + max( 0, epsilon_aum - 1 ) - delta_ra) / 3 );
% delta_ra
...
% end
f(68-1) = C_al - ( N_al * 387 );
% N_al
f(69-1) = N_al_p - ( 1/T_al * (N_als - N_al) );
% N_als
f(70-1) = N_als - ( N_als_eq * xi_ksod * xi_map * xi_at );
% xi_ksod
...
f(73-1) = xi_at - ( xiat_a + xiat_b / (1 + exp(-xiat_c * ((AT1R/AT1R_eq) - xiat_d)) ) );
% hatC_anp
...
%     f(91-1) = Psi_AT2RAA - ( 0.025 * (AT2R_eq - AT2R) + 1 );
    f(91-1) = Psi_AT2RAA - ( Psi_AT2RAA_eq * (0.9 + 0.1 * exp(-1.0 * (AT2R/AT2R_eq - 1))) );
...
%     f(92-1) = Psi_AT2REA - ( 0.01  * (AT2R_eq - AT2R) + 1 );
    f(92-1) = Psi_AT2REA - ( Psi_AT2REA_eq * (0.9 + 0.1 * exp(-1.0 * (AT2R/AT2R_eq - 1))) );
