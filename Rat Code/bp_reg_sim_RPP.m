...
% f(33-1) = P_ra - ( 0.2787 * exp(Phi_co * 0.2281 / SF) - a );
% vas
...
% eta_cdwreab
f(59-1) = eta_cdwreab - ( eta_cdwreab_eq * mu_cdsodreab * mu_adh );
% mu_cdsodreab
...
% end
% % f(65-1) = nu_mdsod - ( 0.2262 + 28.04 / (11.56 + exp((Phi_mdsod - 1.667) / 0.6056)) );
if     strcmp(gender,'male')
    f(65-1) = nu_mdsod - ( 0.2262 + 28.04 / (11.56 + exp((Phi_mdsod - fixed_var_pars(9)) / (0.6056 * SF) )) );
%     f(65-1) = nu_mdsod - ( 0.2262 + 28.04 / (11.56 + exp((Phi_mdsod - 1.637 * SF * 2.500) / (0.6056 * SF * 2.500) )) ); % female
elseif strcmp(gender,'female')
    f(65-1) = nu_mdsod - ( 0.2262 + 28.04 / (11.56 + exp((Phi_mdsod - 1.637 * SF * 2.500) / (0.6056 * SF * 2.500) )) );
%     f(65-1) = nu_mdsod - ( 0.2262 + 28.04 / (11.56 + exp((Phi_mdsod - 1.731 * SF) / (0.6056 * SF) )) ); % male
end
% MODIFY ------------------------------------------------------------------
% nu_rsna
if     t < tchange
    f(66-1) = nu_rsna - ( 1.822 - 2.056 / (1.358 + exp(rsna - 0.8667)) );
elseif t >= tchange
    if   strcmp(scenario,'Denerve') || strcmp(scenario,'Denerve & AT2R-')
        f(66-1) = nu_rsna - ( SSdata(66-1) );
    else
...
% xi_map
if P_ma <= 100
    f(71-1) = xi_map - ( (1/exp(-0.0425 * 100)) * exp(-0.0425 * P_ma) );
else
    f(71-1) = xi_map - ( 1 );
end
% xi_at
...
if   strcmp(scenario,'Denerve & AT2R-')
    alpha = 10;
    f(82-1) = AT2R_p - ( c_AT2R * AngII - log(2)/h_AT2R * AT2R - alpha * AT2R );
else
...
% f(87-1) = Sigma_myo - ( 0.95 + 1.0 / ( 1 + (19/1) * exp(-1.4 * (P_gh - 62)) ) );
...
%     f(90-1) = Psi_AT2RAA - ( 0.7 + 0.3 * exp(-1.3 * (AT2R/AT2R_eq - 1)) );
%     f(90-1) = Psi_AT2REA - ( 0.93 + 0.07 * exp(-0.9 * (AT2R/AT2R_eq - 1)) );
%     f(90-1) = Psi_AT2RAA - ( 1 );
end
% Psi_AT2REA
if     strcmp(gender,'male')
...
%     f(91-1) = Psi_AT2REA - ( 0.8 + 0.2 * exp(-1.4 * (AT2R/AT2R_eq - 1)) );
%     f(91-1) = Psi_AT2RAA - ( 0.85 + 0.15 * exp(-0.85 * (AT2R/AT2R_eq - 1)) );
%     f(91-1) = Psi_AT2REA - ( 1 );
end
