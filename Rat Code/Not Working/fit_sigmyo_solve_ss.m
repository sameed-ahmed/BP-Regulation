...
function [pars_min, ess_min, exitflag, output] = fit_sigmyo_solve_ss

% Turn off warning for clean command window.
% [~, MSGID] = lastwarn();
% warning('off', MSGID)
...
% Sigma_myo = a + b / ( 1 + c * exp(d * (P_gh - 62)) );
a   = 0.35               ; % vertical shift
b   = 5.9                ; % height
d   = -1.1               ; % slope
P_0 = 63.9               ; % horizontal shift in scaleable units
c   = exp(d * (62 - P_0)); % horizontal shift
% a   = random('unif',  0, 1); % vertical shift
% b   = random('unif',  0,20); % height
% d   = random('unif',-50, 0); % slope
% P_0 = random('unif', 55,70); % horizontal shift in scaleable units
% c   = exp(d * (62 - P_0))  ; % horizontal shift
...
a = pars_min(1); b = pars_min(2); d = pars_min(3); P_0 = pars_min(4);
c = exp(d * (62 - P_0));
pars_min = [a; b; c; d];

save_data_name = sprintf('%s_pars_est_sigmamyo.mat', gender{gg});
save(save_data_name, 'pars_min', 'ess_min', 'exitflag', 'output', 'CPUtime')

end % gender

% % Turn warning back on for other scripts.
% warning('on', MSGID)
...
% MAP whole range
MAP_lower = (ARR(1)-30         : deltaMAP : ARR(1)-deltaMAP  )';
MAP_upper = (ARR(end)+deltaMAP : deltaMAP : ARR(end)+30      )';
MAP_range = [MAP_lower; ARR; MAP_upper];

%% Form GFR actual

% GFR autoregulatory range
% The -3*deltaGFR is due to baseline value being at BP=105, while
% autoregulatory range starting at BP=90.
GFR_act = zeros(middle_num,1) + GFR_baseline - 3*deltaGFR;
for i = 1:middle_num
    GFR_act(i) = GFR_act(i) + (i-1)*deltaGFR;
end
% GFR whole range
GFR_act_lower = (1-fact) * GFR_act(1  );
GFR_act_upper = (1+fact) * GFR_act(end);
GFRslope_lower = fact*GFR_act(1)/30; GFRslope_upper = fact*GFR_act(end)/30;
GFR_act_lower = (GFR_act_lower                 : 5*GFRslope_lower : GFR_act(1)-5*GFRslope_lower)';
GFR_act_upper = (GFR_act(end)+5*GFRslope_upper : 5*GFRslope_upper : GFR_act_upper              )';
GFR_act = [GFR_act_lower; GFR_act; GFR_act_upper];

%% Modify MAP and GFR calculated

% Retrieve GFR calculated and steady state.
[MAP, GFR] = solve_ss_Pma(pars,pars_est,SSdata_IG);
...
for i = length(MAP):-1:1
    if MAP(i) > MAP_range(end) || MAP(i) < MAP_range(1)
        MAP(i) = '';
        GFR(i) = '';
    end
end
if isempty(MAP) || min(MAP) > MAP_range(1) || max(MAP) < MAP_range(end)
%     MAP
    err = 1;
    return;
end
...
GFR_cal_lower = GFR(1:5:26); GFR_cal_upper = GFR(106:5:131);
GFR_cal = GFR(middle_ind);
GFR_cal = [GFR_cal_lower; GFR_cal; GFR_cal_upper];
...
% =========================================================================
% Solve steady state for perturbed MAP.
% =========================================================================

function [MAP, GFR] = solve_ss_Pma(pars,pars_est,SSdata_IG)

%% Setup

P_ma_range = linspace(60,190,141);
iteration = length(P_ma_range);

% X = (variable, iteration)
X = zeros(82,iteration);
...
[SSdata, ~, ...
 exitflag, output] = fsolve(@(x) bp_reg_solve_Pma(t,x,x_p0,pars,pars_est,P_ma_ss), ...
                            x0, options);
...
% =========================================================================
% Model function for steady state solution.
% =========================================================================

function f = bp_reg_solve_Pma(t,x,x_p,pars,pars_est,P_ma_ss)

% pars_est = [a; b; d; P_0];
a = pars_est(1); b = pars_est(2); d = pars_est(3); P_0 = pars_est(4); 
c = exp(d * (62 - P_0));
...
% ESTIMATE ----------------------------------------------------------------
% Sigma_myo
f(78) = Sigma_myo - ( a + b / ( 1 + c * exp(d * (P_gh - 62)) ) );
% ESTIMATE ----------------------------------------------------------------
% Psi_AT1RAA
