...
function f = bp_reg_solve(t,x,x_p,pars,drugs)
...
f(48) = N_adhs - ( (max( 0, C_sod - 141) + max( 0, epsilon_aum - 1 ) - delta_ra) / 3 );
% delta_ra
...
f(50) = Phi_twreab - ( 0.025 * SF - 0.001 * SF / (mu_al * mu_adh) + 0.8 * Phi_gfilt + 0.05*tanh(2*(mu_Na-1)) * Phi_gfilt );
% mu_al - rat
...
% f(52) = mu_adh - ( -5.1687 + 6.3 / (1 + 0.14545 * exp(-0.48 * C_adh)) ); 
f(52) = mu_adh - ( -11.8687 + 13 / (1 + 0.04137 * exp(-0.35 * C_adh)) ); 
% Phi_u - rat
...
%     f(56) = nu_mdsod - ( 0.2262 + 28.04 / (11.56 + exp((Phi_mdsod - 1.637 * SF * 2.500) / (0.6056 * SF * 2.500) )) ); % female
elseif strcmp(gender,'female')
    f(56) = nu_mdsod - ( 0.2262 + 28.04 / (11.56 + exp((Phi_mdsod - 1.637 * SF * 2.500) / (0.6056 * SF * 2.500) )) );
%     f(56) = nu_mdsod - ( 0.2262 + 28.04 / (11.56 + exp((Phi_mdsod - 1.731 * SF) / (0.6056 * SF) )) ); % male
end
% nu_rsna
...
%     f(58) = C_al - ( N_al * 85      );
% elseif strcmp(gender,'female')
%     f(58) = C_al - ( N_al * 69.1775 );
% end
if     strcmp(gender,  'male')
    f(58) = C_al - ( N_al * 395.3 );
elseif strcmp(gender,'female')
    f(58) = C_al - ( N_al * 379.4 );
%     f(58) = C_al - ( N_al * 395.3 ); % male
end
% N_al
...
% AngII
f(71) = AngII_p - ( k_AngII + (c_ACE + c_Chym) * AngI - (c_ACE2 + c_IIIV + c_AT1R + c_AT2R) * AngII - log(2)/h_AngII * AngII );
% AT1R
f(72) = AT1R_p - ( c_AT1R * AngII - log(2)/h_AT1R * AT1R );
% AT2R
f(73) = AT2R_p - ( c_AT2R * AngII - log(2)/h_AT2R * AT2R - alpha * AT2R );
% Ang17
...
% Sigma_myo
f(78) = Sigma_myo - ( 0.9 + 1.0 / ( 1 + (9/1) * exp(-0.9 * (P_gh - 62)) ) );
% f(78) = Sigma_myo - ( 0.95 + 1.0 / ( 1 + (19/1) * exp(-1.4 * (P_gh - 62)) ) );
% f(78) = Sigma_myo - ( 5 * (P_gh / 62 - 1) + 1 );
% Psi_AT1RAA
...
%     f(81) = Psi_AT2RAA - ( 0.7 + 0.3 * exp(-1.3 * (AT2R/AT2R_eq - 1)) );
%     f(81) = Psi_AT2REA - ( 0.93 + 0.07 * exp(-0.9 * (AT2R/AT2R_eq - 1)) );
%     f(81) = Psi_AT2RAA - ( 1 );
end
% Psi_AT2REA
if     strcmp(gender,'male')
...
%     f(82) = Psi_AT2REA - ( 0.8 + 0.2 * exp(-1.4 * (AT2R/AT2R_eq - 1)) );
%     f(82) = Psi_AT2RAA - ( 0.85 + 0.15 * exp(-0.85 * (AT2R/AT2R_eq - 1)) );
%     f(82) = Psi_AT2REA - ( 1 );
end
