...
function f = bp_reg_sim_Pma(t,x,x_p,pars,P_ma_input,tchange,inc)

%% Retrieve parameters by name.

if     t < tchange
    P_ma_input = P_ma_input;
elseif t >= tchange
    P_ma_input = P_ma_input + inc;
end
...
% alpha_map
f(2 ) = alpha_map - ( 0.5 + 1 / (1 + exp((P_ma_input - 100) / 15)) );
% alpha_rap
...
% Phi_rb
f(6 ) = Phi_rb - ( P_ma_input / R_r );
% Phi_gfilt
f(7 ) = Phi_gfilt - ( P_f * C_gcf );
% P_f
f(8 ) = P_f - ( P_gh - (P_B + P_go) );
% P_gh
f(9 ) = P_gh - ( P_ma_input - Phi_rb * R_aa );
...
% a_auto
f(43-1) = a_auto - ( 3.0042 * exp(-0.011 * P_ma_input) );
% a_chemo
...
f(50-1) = Phi_twreab - ( 0.025 * SF - 0.001 * SF / (mu_al * mu_adh) + 0.8 * Phi_gfilt + 0.05*tanh(5*(mu_Na-1)) * Phi_gfilt );
% mu_al - rat
...
% xi_map
if P_ma_input <= 100
    f(62-1) = xi_map - ( 70.1054 * exp(-0.0425 * P_ma_input) );
else
    f(62-1) = xi_map - ( 1 );
end
% xi_at
...
% Sigma_myo
% f(78-1) = Sigma_myo - ( 0.348582347120430 + 5.914000532919649 / ( 1 + 8.078661756826412 * exp(-1.105825269473267 * (P_gh - 62)) ) );
f(78-1) = Sigma_myo - ( 5 * (P_gh / 62 - 1) + 1 );
% Psi_AT1RAA
