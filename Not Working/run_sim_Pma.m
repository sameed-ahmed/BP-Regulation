...
P_ma_bl_f = 100.6658;
if     strcmp(gender{gg}, 'male')
    P_ma_input = P_ma_bl_m;
elseif strcmp(gender{gg}, 'female')
    P_ma_input = P_ma_bl_f;
end
inc = 0.1;
...
options = odeset('MaxStep',0.01); % default is 0.1*abs(t0-tf)
...
[t,x] = ode15i(@(t,x,x_p) ...
               bp_reg_sim_Pma(t,x,x_p,pars,P_ma_input,tchange,inc), ...
               tspan, x0, x_p0, options);
...
P_ma_bl_m = [P_ma_bl_m * ones(1,length(t_m(1:index_m-1))), ...
             P_ma_bl_m * ones(1,length(t_m(index_m:end))) + inc];
P_ma_bl_f = [P_ma_bl_f * ones(1,length(t_f(1:index_f-1))), ...
             P_ma_bl_f * ones(1,length(t_f(index_f:end))) + inc];
X_m = [X_m(1:40,:); P_ma_bl_m; X_m(41:end,:)];
X_f = [X_f(1:40,:); P_ma_bl_f; X_f(41:end,:)];
