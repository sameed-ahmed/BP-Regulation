% This script varies the renal perfusion pressure and then calculates the 
% steady state solution of the system for each inputted value. To avoid the
% solver not converging, the initial guess for solution to the system is 
% taken as the previous solution value. That is, IG_i = SOL_i-1.
% 
% All variables are then plotted versus the relative change in input.

function solve_ss_RPP
...
% RPP_range = [scenario, iteration]
RPP_range_m = RPP_m * iter_range;
RPP_range_f = RPP_f * iter_range;
...
% Vary renal perfusion pressure.
if     strcmp(gender{gg}, 'male')
    if     strcmp(change{cc}, 'decrease')
        RPP_input = RPP_range_m(ss,iteration-iter+1);
    elseif strcmp(change{cc}, 'increase')
        RPP_input = RPP_range_m(ss,iteration+iter-1);
    end
elseif strcmp(gender{gg}, 'female')
    if     strcmp(change{cc}, 'decrease')
        RPP_input = RPP_range_f(ss,iteration-iter+1);
    elseif strcmp(change{cc}, 'increase')
        RPP_input = RPP_range_f(ss,iteration+iter-1);
    end
end
...
% options = optimset(); %options = optimset('MaxFunEvals',8200+1000);
...
%     f(i) = figure('pos',[750 500 650 450]);
    ...
    for j = 1:last_plot
        ...
        plot(s(i,j), xscale_m,X_m((i-1)*15 + j,:,1),'b', ...
                     xscale_f,X_f((i-1)*15 + j,:,1),'r');
        
        xlim([xlower, xupper])
        ylim([ylower((i-1)*15 + j), yupper((i-1)*15 + j)])
        
        xlabel('$RPP$'           , 'Interpreter','latex', 'FontSize',15)
...
% ylabel(names(27), 'Interpreter','latex', 'FontSize',22, 'FontWeight','bold')
% % hold all
